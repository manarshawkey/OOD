...
public:
	InstFactory();
	~InstFactory();
	void parse(std::string, std::vector<instruction*>&);
private:
	void create_inst_object(std::stringstream&, std::vector<instruction*>& inst_mem);
};
InstFactory::InstFactory() {
//	std::cout << "Factory constructor\n";
}
void InstFactory::parse(std::string path, std::vector<instruction*>& inst_mem) {
	std::ifstream input;
	input.open(path);
	std::string line;
	...
	while (getline(input, line, '\n')) {
		...
		create_inst_object(inst_line, inst_mem);
	}
}
InstFactory::~InstFactory(){}
void InstFactory::create_inst_object(std::stringstream& inst_line, std::vector<instruction*>& inst_mem) {
	std::string word, word1;
	inst_line >> word;
	int in, out, in2;
	//neg in, out  //in can be address   @in
	//ass in1, out1
	if (word == "ASS" || word == "NEG") {  //changed: flg
		bool flg = 0;
		inst_line >> word1;
		...
		out = std::stoi(word1);
		if (word == "ASS")
			inst_mem.push_back(new AssignInst(in, out, flg));
		else
			inst_mem.push_back(new NegInst(in, out, flg));
	}else
	if (word == "MUL" || word == "ADD" || word == "LE") {  //changed
		...
		if (word == "ADD") inst_mem.push_back(new AddInst(in, in2, out, flg1, flg2));
		else if (word == "MUL") inst_mem.push_back(new MultInst(in, in2, out, flg1, flg2));
		else inst_mem.push_back(new LessEqInst(in, in2, out, flg1, flg2));
	}
	else if (word == "READ") {
		inst_line >> word1;
		...
		word1.erase(0, 1);
		out = std::stoi(word1);
		inst_mem.push_back(new ReadInst(out));
	}
	else if (word == "WRITE") {
		bool flg = 0;
		inst_line >> word1;
		...
		inst_mem.push_back(new WriteInst(out, flg));
	}
	else if (word == "HALT") {
		...
		inst_mem.push_back(new JumpInst(addr));
	}
	else if (word == "JUMP0") {
		inst_line >> word1;
		if (word1[0] == '@') {
		...
		word1.erase(0, 1);
		out = stoi(word1);
		inst_mem.push_back(new Jump0Inst(in, out));
	}
	else {
